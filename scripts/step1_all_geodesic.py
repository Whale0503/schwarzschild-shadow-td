...
def find_outer_horizon(M, r_min=1e-6, r_max=50.0, N=20000):
    r_grid = np.linspace(r_min, r_max, N)
    B_vals = B_func(r_grid, M)
    ...
    for i in range(len(r_grid) - 1):
        if B_vals[i] * B_vals[i+1] < 0:
            ...
            root = brentq(B_func, r1, r2, args=(M,))
            roots.append(root)

    if len(roots) == 0:
        return None   # 无视界（裸奇点或无黑洞）
...
if __name__ == "__main__":
    ...
    r_h = find_outer_horizon(M)
    if r_h is None:
        print("No horizon found. Exit computation.")
        sys.exit(0)   # 立即、安全退出整个程序
    else:
        print(f"Outer horizon r_h = {r_h:.6f}")
